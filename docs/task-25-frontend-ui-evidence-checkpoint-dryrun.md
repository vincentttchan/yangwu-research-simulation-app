...
Status: partial pass with one deployment-environment finding.
...
Task 25 confirms the front-end event coverage and production API acceptance, but it does not yet confirm that the rows are visible in the intended `yangwu-research-lkkc-2026` Supabase project.

Most likely finding:

Vercel Production environment variables for `SUPABASE_URL` and/or `SUPABASE_SECRET_KEY` appear to point to a different Supabase project or branch than the Supabase dashboard project currently being checked.

This is an environment-target mismatch, not a front-end instrumentation failure.
...
| Intended Supabase export visibility | Blocked | `research_event_log_long_export` returned `0 row` for the production UI session in the checked project. |
| Privacy QA | Pass in checked project | `research_privacy_exception_export` returned `0 row`. |

## Required Next Step

Before formal pilot data collection, verify and align the Vercel Production values for:

- `SUPABASE_URL`
- `SUPABASE_SECRET_KEY`
- `SUPABASE_SCHEMA`
- `RESEARCH_BACKEND_ENABLED`
- `RESEARCH_COHORT`
- `APP_VERSION`

The next verification should repeat Task 25 after environment alignment and confirm that `source_opened` and `checkpoint_submitted` appear in `research_event_log_long_export` in the intended `yangwu-research-lkkc-2026` project.
